...
public class CommandsGenerator {
    ...
    public AvailableCmds getCommand(String text) {
        ...
        /*
         * On teste si word1 est une commande en utilisant la methode isCommand
         * se trouvant dans la classe GoodCommands on retourne la commande
         * correspondante
         */
        if (this.cmd.isACommand(word1) && (token.countTokens() == 0)) {
            this.validCmd = new AvailableCmds(this.cmd.getCommands(word1),
                    word2, word3);
    ...
    public void displayAllCommands() {
        ...
        System.out.println(
                "look:        permet d'afficher les objets et les sorties présentes dans la pièce courante");
        System.out.println(
                "\t---> look : affiche la description de votre position,");
        System.out.println(
                "\t---> look nomObjet : affiche la description de l'objet");

        System.out.println("go:          permet de se deplacer");
        System.out.println(
                "\t---> go direction : permet de se rendre dans la direction indiquee");

        System.out.println(
                "use:         permet d'utiliser un objet de son inventaire.");
        System.out.println(
                "\t---> use Cle direction : permet d'utiliser la cle pour deverouiller la porte menant a ce lieu");
        System.out.println(
                "\t---> use arme ennemi : permet d'utiliser l'arme sur l'ennemi");

        System.out.println(
                "take:        permet de ramasser un objet dans la pièce courante.");
        System.out.println("\t---> take arme : permet de rammasser l'arme");

        System.out.println(
                "attack:      Cette commande ne s'utilise que lors d'un combat.");
        System.out.println(
                "\t---> attack : permet d'attaquer a mains nus votre adversaire.");
