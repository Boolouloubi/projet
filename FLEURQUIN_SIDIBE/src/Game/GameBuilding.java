...
import Items.Weapon;
import Places.Exit;
import Places.LockDoor;
import Places.Place;
import Places.World;

/**
 * This class is used to build the game by creating the world and its places,
...
public class GameBuilding {
    ...
    public void scenario() {
        ...
        System.out.println(
                "sur le fameux Capitaine Nemo et son sous-marin le Nautilus.");
        System.out.println(
                "Lors d'une escale en Jamaique, vous faites la rencontre d'un equipage et de son capitaine qui pretend");
        System.out.println(
                "avoir des informations sur la localisation de la celebre machine et vous decidez de le rejoindre.");
        System.out.println(
                "Apres plusieurs jours en mer, il s'avere que les informations de ce mysterieux capitaine etait juste,");
        System.out.println(
                "le Nautilus se trouve au fond de l'ocean juste en dessous de votre navire et vous vous portez volontaire");
        System.out.println("pour explorez la machine de guerre.\n");
        System.out.println(
                "Le but de votre plongee est de recuperer le livre de bord du Nautilus dans lequel ");
        System.out.println(
                "le Capitaine Nemo aurait decrit son voyage et surtout l'emplacement de tous ses tresors.");
    }
    ...
    public void menu() {
        ...
        while (true) {
            ...
            case "1":
                ...
                clearScreen();
                System.out.println(
                        "Vous venez de penetrer dans l'enceinte du Nautilus.");
                System.out.println(
                        "Vous vous trouvez dans une piece mysterieuse dont la porte est verrouillee.");
            ...
            default:
                System.out.println(
                        "Mauvais choix, Veuillez ressayer de nouveau.");
                System.out.println("\n\nAppuyez sur \"entree\"...");
                sc.nextLine();
                clearScreen();
                break;
    ...
    public void createTheWorld() {
        ...
        Exit startPlaceToCouloir1 = new LockDoor("startPlaceToCouloir1",
                this.startPlace, couloir1); // LockDoor
        ...
        Exit salleMachineToPieceSecrete = new LockDoor(
                "salleMachineToPieceSecrete", salleMachines, pieceSecrete);
        new Exit("couloir3ToSalleMachine", couloir3, salleMachines);
        Exit couloir3ToCuisine = new LockDoor("couloir3ToCuisine", couloir3,
                cuisine); // LockDoor
        ...
        Exit couloir4ToCockpit = new LockDoor("couloir4ToCockpit", couloir4,
                cockpit); // LockDoor
        new Exit("couloir4ToSalleSport", couloir4, salleSport);
        new Exit("salleSportToVestiaires", salleSport, vestiaires);

        // ********************************* The items
        this.cahierDeBord = new Book("cahier_de_bord",
                "C'est le cahier de bord du Capitaine Nemo");

        Item cleSalleDepart = new Key("cle_salle_depart",
                "C'est la cle de la salle de depart");
        ...
        Item clePieceSecrete = new Key("cle_piece_secrete",
                "C'est la cle de la piece secrete");
        ((LockDoor) salleMachineToPieceSecrete).setKey((Key) clePieceSecrete);

        Item oxygene = new Oxygen("oxygene",
                "C'est une capsule d'oxygene, utilisez la pour restaurer votre energie");
        Item munitions = new Arrow("munitions",
                "Ce sont des fleches pour le harpon", 15);
        Item couteau = new Knife("couteau",
                "C'est un couteau en or, utilisez le pour vous defendre", 25);
        Item harpon = new Harpon("harpon",
                "C'est un harpon, utilisez-le pour vous defendre", 40);
    ...
    public void play() {
        ...
        Scanner sc = new Scanner(System.in);
        String input = "";
        while ((!this.hero.hasItem(cahierDeBord)) && (this.hero.isAlive())
                && (this.hero.getOxygen() > 0)) {
            this.clearScreen2();
    ...
    public void interpretCmds(String input) {
        ...
        if (!cmd.isCommand()) {
            ...
            case attack:
                System.out.println(
                        "Vous ne pouvez attaquer que lors d'un combat.\n");
                break;
            case look:
    ...
    public void getDescription(AvailableCmds cmd) {
        if (!cmd.hasSecondWord()) {
            ...
            if (cmd.hasThirdWord()) {
                System.out.println(
                        "Combinaison impossible. Veuillez consulter l'aide!\n");
            } else {
    ...
    public void takeItem(AvailableCmds cmd) {
        ...
        if (!cmd.hasSecondWord()) { // If cmd hasn't a second word
            ...
            if (cmd.hasThirdWord()) {
                ...
                Item item = this.hero.getPosition()
                        .getItem(cmd.getSecondWord());
                if (item == null) {
    ...
    public void move(AvailableCmds cmd) {
        if (!cmd.hasSecondWord() || cmd.hasThirdWord()) { // if the command
            // hasn't a second
            // word
            ...
            if (placeToGo == null) {
                ...
                if (e == null) {
                    System.out.println(
                            "Vous ne pouvez pas allez a cet endroit...");
                } else {
                    if (e instanceof LockDoor) {
                        if (((LockDoor) e).isLock()) {
                            System.out
                            .println("Cette porte necessite une cle.");
                            System.out.println(
                                    "Utilisez la cle pour la deverouiller...");
                        } else {
        ...
        if (!cmd.hasSecondWord()) {
            ...
            if (item == null) {
                ...
                if ((item instanceof Knife)) { // If the hero wants use a knife
                    if (!cmd.hasThirdWord()) {
                        System.out.println(
                                "Sur quoi comptez-vous utiliser cet objet??");
                    } else {
                        String thirdWord = cmd.getThirdWord();
                        Character ennemy = this.hero.getPosition()
                                .getCharacter(thirdWord);
                        if (ennemy == null) {
                            System.out.println(
                                    "Cette creature n'est pas en ce lieu...");
                        } else if (ennemy instanceof Hero) {
                            System.out.println(
                                    "Vous ne pouvez pas vous suicider!!!");
                            System.out.println(
                                    "Votre quete n'est pas encore accomplie...");
                        } else if (ennemy instanceof Nemo) {
                            System.out.println(
                                    "Cette creature est un fantome...");
                        } else {
                ...
                else if (item instanceof Arrow) { // If the hero wants use an
                    // arrow
                    if (!this.hero.hasItem(item)) {
                        ...
                        if (!cmd.hasThirdWord()) {
                            ...
                            if (ennemy == null) {
                                ...
                                this.hero.attack(ennemy, (Weapon) item);
                                this.hero.removeItem(item); // We remove one
                                // arrow from the
                                // inventory
                            }
                        }
                ...
                else if (item instanceof Harpon) { // If the hero wants use the
                    // harpoon
                    Arrow arrow = null;
                    ...
                    if (arrow == null) {
                        ...
                        if (!cmd.hasThirdWord()) {
                            ...
                            if (ennemy == null) {
                                ...
                                this.hero.attack(ennemy, (Weapon) item);
                                this.hero.removeItem(arrow); // We remove one
                                // arrow from the
                                // inventory
                            }
                        }
                ...
                else if (item instanceof Oxygen) { // If the hero wants use an
                    // oxygen caps.
                    if (!cmd.hasThirdWord()) {
                    ...
                    if (!cmd.hasThirdWord()) {
                        ...
                        Place place = this.hero.getPosition()
                                .getExit(cmd.getThirdWord());
                        Exit e = this.hero.getPosition().getExit(place);
                        if (e == null) {
                            System.out.println("Cette sortie n'existe pas.");
                        } else {
                            if (e instanceof LockDoor) { // If it's a door with
                                // lock
                                if (((LockDoor) e).getKey() == item) { // If
                                    // it's
                                    // the
                                    // same
                                    // key
                                    ((LockDoor) e).unlock();
                                    System.out.println(
                                            "La porte est deverouillee...");
                                } else { // If it isn't the same key
                                    if (!((LockDoor) e).isLock()) // If the door
                                        // is already
                                        // opened
                                        System.out.println(
                                                "Cette sortie n'est pas verouillee");
                                    else
                                        System.out.println(
                                                "Ce n'est pas la bonne cle...");
                                }
                            } else {
                                System.out.println(
                                        "Cette porte est deja ouverte");
                            }
                        }
                    }
    ...
    public void startFighting(Character champion, Ennemy outsider) {
        ...
        Scanner sc = new Scanner(System.in);
        int round = 1;
        System.out.println("\nVous etes attaques par " + outsider.getName()
        + ", defendez-vous!!!\n");
        while (champion.isAlive() && outsider.isAlive()) {
            System.out
            .println("------------- ROUND " + round + " ------------");
            System.out.println(champion.getName() + " : " + champion.getHP()
            + " points de vie");
            System.out.println(outsider.getName() + " : " + outsider.getHP()
            + " points de vie\n");
            ...
            if (input.equals(Commands.attack.toString())) {
                champion.attack(outsider, null); // null ==> champion uses its
                // harms
            } else {
            ...
            if (outsider.isAlive()) {
                System.out.println(outsider.getName()
                        + " viens de vous attaquer... Defendez-vous\n");
            }
            round++;
        }
        if (!outsider.isAlive()) {
